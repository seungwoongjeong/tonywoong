# 제 1,2 부터 100사이의 모든 짝수를 출력하는 반복 루프를 작성해보자.
# x%2한 결과가 0이면 x는 짝수이다.
 for i range(2,101):
     if i%2 == 0 :
         print(I, end = " ")
         
# 2. 복리이자율 7%로 1000만원을 저축했을 경우 2000만원이 되는데 몇년이 걸리는지 계산하기 위해 아래의 코드를 작성하였다.
# 잘못된 점은 없는지 체크하라
...
ans = 0
while ans ! - 3*9 :
    ans = int(input("3 x 9s는?"))
print("맞았습니다.")

# 5. 사용가자 입력한 정수의 합을 계산하는 프로그램을 작성하라. 사용자가 0을 입력하기전까지 정수를 계속하여 읽도록 하자.
# 무한루프를 작성하고 사용자가 입력한 값이 0이면 break 문장을 실행하여 반복 루프를 종료한다. 무한루프는 whil True: 이다.
...
#    별들은 시작 각도가 약간씩 다르다.
# 별을 그리는 코드를 반복한다. 하나의 별을 그리는 반복이 끝나면 lefr*10을 실행한다.
...
# 참고 : 1부터 100사이의 난수를 발생하려면 r = random.randint(1,100)을 사용하면 됩니다. 첫 부분에 random도 import
# 해야 하고요. 원의 중심, 원의 반지름을 모두 난수로 설정합니다.
...
for j in range (1, 10) : 
    ...
    y = random.randist(-200, 200)
    r = random.randint(10, 200)
    t.goto(x, y)
    t.down()
    t.circle(r)
    t._screen.exitonlick()  